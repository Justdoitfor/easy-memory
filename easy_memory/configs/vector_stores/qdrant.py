...
class QdrantConfig(BaseModel):
    ...
    def check_host_port_or_path(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        host, port, path, url, api_key = (
            values.get("host"), values.get("port"), values.get("path"),
            values.get("url"), values.get("api_key"),
        )
    ...
    def validate_extra_fields(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if extra_fields:
            raise ValueError(
                f"Extra fields not allowed: {', '.join(extra_fields)}. Please only provide the supported fields.")
        return values
