...
def _strip_generic_ending(toks):
    if len(toks) <= 1:
        return toks
...
def extract_entities(text):
    from easy_memory.utils.spacy_models import get_nlp_full
    nlp = get_nlp_full()
    if nlp is None:
        return []
    doc = nlp(text)
...
def extract_entities_batch(texts, batch_size=32):
    if not texts:
        return []
    from easy_memory.utils.spacy_models import get_nlp_full
    nlp = get_nlp_full()
...
def _extract_entities_from_doc(doc):
    entities = []
    text = doc.text
    tokens = list(doc)
    ...
    while i < len(tokens):
        ...
        if is_cap and not is_label and tok.pos_ in {"PROPN", "NOUN", "ADJ"}:
            ...
            while j < len(tokens):
                t = tokens[j]
                if (t.text and t.text[0].isupper()) or t.text.lower() in {"'s", "of", "the", "in", "and", "for", "at",
                                                                          "is"}:
                    seq.append((t, j))
                    j += 1
                else:
    ...
    for chunk in doc.noun_chunks:
        ...
        for group in groups:
            ...
            content = [
                t for t in group
                if t.pos_ not in {"DET", "PRON", "PUNCT", "PART", "ADP", "SCONJ", "NUM"} and (
                            t.pos_ == "ADJ" or not t.is_stop)
            ]
            if not content:
                continue
            ...
            if compound_toks:
                ...
                filtered = _strip_generic_ending(
                    [t for t in content if
                     not ((t.pos_ == "ADJ" or t.dep_ == "amod") and t.lemma_.lower() in _NON_SPECIFIC_ADJ)]
                )
                if filtered:
                    phrase = _lemmatize_compound(filtered)
                    if len(phrase) > 3 and " " in phrase:
