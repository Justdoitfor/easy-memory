...
class RedisDB(VectorStoreBase):
        ...
        return [
            type(
                ...
                (),
                {
                    ...
                    "payload": json.loads(self.client.json().get(self._doc_key(getattr(doc, "id", ""))) or "{}"),
                },
            )()
    ...
    def update(self, vector_id, vector=None, payload=None):
        ...
        if vector:
            self.client.json().set(key, "$.embedding", self.np.array(vector, dtype=self.np.float32).tobytes().hex())
        if payload:
